...
- **Video (portrait):** the FPGA scans the **native 288×224 raster** and never rotates; portrait is set in `video.json` (`rotation: 90`). Decode RGB through the 82s123 palette + 82s126 lookup PROMs into `video_rgb[23:0]`. `video.json` `width`/`height` must equal the `video_de` window — **add a scaler acceptance check at ~60.6 Hz portrait, using `video_skip` as the lever.**
...
| **3** | Video | Palette/lookup PROM decode + tile/sprite raster on the APF bus; `rotation:90`. **Pac-Man attract + maze render correctly in portrait, correct colours.** |
