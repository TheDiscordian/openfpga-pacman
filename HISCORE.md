...
| 6 | Stale-RAM capture on reload | a **warm core reload** leaves the prior session's bytes in work RAM; if the first snapshot runs before the game re-inits the cell, it captures that garbage (invalid BCD like `90c0f020`) into the `.sav`. Next reload re-injects the garbage → the game's beat-compare reads an impossible high score, so a real beat never registers or redraws, and the bad value snapshots forward forever | `seen`: a `scan_uni` work-RAM value region is only snapshotted after it has been observed at its cold default at least once this session (set in `S_ZS` when the whole region reads `sval`). Display rows (`guard_disp`, offset < `0x400`) are exempt — their digit guard (#2) already covers stale video RAM. |
